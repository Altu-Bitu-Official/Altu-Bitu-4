...
int calcDiff(vector<int>& start_number, int n, int k) { // x와 n의 다른 bit 개수를 세주는 함수
	int cnt = 0;
	for (int i = 0; i < k; i++) {
		unsigned char diff = (start_number[i] ^ number_bit[n % 10]); // XOR 연산을 해주면 비트가 다른 부분만 1이 남는다
...
//1. LED의 각 자리를 비트로 표현
//2. x와 n또한 각 자리를 비트로 표현하고 xor연산을 통해 서로 다른 비트의 개수를 구하기
//3. 다른 비트의 개수가 1이상 p이하면 조건 만족
