...
void rotateBelt(deque<info> &belt, int n) {
    belt.push_front(belt.back());   // belt의 마지막 원소를 belt의 앞에 넣음
    belt.pop_back();    // belt의 마지막 원소를 빼냄
...
void moveRobot(deque<info> &belt, int n) {
    for (int i = n - 2; i >= 0; i--) { // 배열 접근 가능 범위가 0 ~ n-1 인데 다음 칸과 비교해야 하니까 0 ~ n-2
        if (belt[i].is_on && (!belt[i + 1].is_on) && (belt[i + 1].power >= 1)) {
            ...
            belt[i].is_on = false;  // 지금 칸에 로봇 존재 안함
            belt[i + 1].is_on = true;   // 다음 칸에 로봇 존재함
...
void putRobot(deque<info> &belt) {
    if (!belt[0].is_on && belt[0].power >= 1) {
        ...
        belt[0].is_on = true;   // 올리는 칸에 로봇이 존재함
...
bool checkFinish(deque<info> &belt, int n, int k) {
    ...
    for (int i = 0; i < 2 * n; i++) {   // i가 0부터 2n-1까지 2n번 동안
        if (belt[i].power == 0) {   // 해당 칸의 내구도가 0이면
            count++;    // count를 +1한다
        }
    }

    if (count >= k) {   // count가 역치인 k 이상이면
        return true;    // true 반환
    }
    return false;   // count가 역치 k 보다 작다면
}
...
int solution(deque<info> &belt, int n, int k) {
    ...
    while (true) {
        ...
        // 내구도 체크하기
        if (checkFinish(belt, n, k)) {  // 내구도 0인 칸이 k개 이상이면
            return step;    // 진행한 단계의 개수를 반환한다
        }
        step++; // 단계 +1
    }
}
...
int main() {
    ...
    for (int i = 0; i < 2 * n; i++) {   // i가 0부터 2n-1까지 2n번 반복
        cin >> belt[i].power;   // 컨베이어 벨트의 내구도 입력받고
        belt[i].is_on = false;  // 컨베이버 벨트의 로봇 존재 여부를 false로 초기화
    }
    // 연산 & 출력
    cout << solution(belt, n, k);   // 솔루션 함수를 이용해 연산 및 출력을 한다
}