...
struct info { // 내구도와 로봇 존재 여부, 코딩의 편의를 위해 자료 구조로 만듦
    int power; //내구도
    bool is_on; //로봇 존재 여부
};
...
void rotateBelt(deque<info>& belt, int n) {
    belt.push_front(belt.back()); //맨뒤에 있는 물건을 빼서 앞에 배치
    belt.pop_back(); //(앞으로 갔으므로) 뒤에 있는 물건 pop
...
void putRobot(deque<info>& belt) {
    if (!belt[0].is_on && belt[0].power >= 1) {
        ...
        belt[0].is_on = true; //벨트에 올릴 수 있으므로 true로 변경
        belt[0].power--;
    }
}
...
bool checkFinish(deque<info>& belt, int n, int k) {
    ...
    if (count >= k) {
        return true; //내구도가 0이하인 칸 k개 미만
    }
    return false; //내구도가 0이하인 칸 k개 이상
}
