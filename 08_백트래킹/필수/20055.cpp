...
void rotateBelt(deque<info> &belt, int n) {
    belt.push_front(belt.back()); //벨트에 가장 뒤쪽을 가장 앞에 푸쉬
    belt.pop_back();// 가장 뒤쪽을 앞에 push 했으므로 가장 뒤쪽 pop하여 없애줌
...
void moveRobot(deque<info> &belt, int n) {
    for (int i = n - 2; i >= 0; i--) { // 배열 접근 가능 범위가 0 ~ n-1 인데 다음 칸과 비교해야 하니까 0 ~ n-2
        if (belt[i].is_on && (!belt[i + 1].is_on) && (belt[i + 1].power >= 1)) {
            ...
            belt[i].is_on = false; //현재 위치에서 로봇이 이동하므로 현재 위치는 false로 만들어줌
            belt[i + 1].is_on = true; // 로봇이 다음 위치로 이동했으므로 다음 위치는 true로 만들어줌
            belt[i + 1].power--; //로봇이 이동했으므로 해당 칸의 내구도 -1
        }
        belt[n - 1].is_on = false; // 로봇이 내리는 위치
    }
}
...
void putRobot(deque<info> &belt) {
    if (!belt[0].is_on && belt[0].power >= 1) {
        ...
        belt[0].is_on = true; //로봇이 존재하므로 true로 만들어줌
        belt[0].power--; //로봇을 올렸으므로 내구도 -1
    }
}

/* 벨트의 내구도 체크 */
bool checkFinish(deque<info> &belt, int n, int k) {
    int count = 0; //내구도가 0이 된 칸의 갯수 저장할 변수
    for (int i = 0; i < 2 * n; i++) {
...
int solution(deque<info> &belt, int n, int k) {
    ...
    while (true) {
        ...
        // 내구도 체크하기
        if (checkFinish(belt, n, k)) { //true가 리턴되면 step 리턴 후 해당 함수 종료됨
            return step;
        }
        step++; //단계 +1
    }
...
int main() {
    // 입력
    int n, k; //길이가 n, 종료 기준인 내구도가 0인 칸 갯수
    cin >> n >> k; //입력받음
    deque<info> belt(2 * n); // 컨베이어 벨트의 내구도와 로봇 존재 여부 저장
    for (int i = 0; i < 2 * n; i++) {
        cin >> belt[i].power; //내구도 입력받아 belt의 power에 저장
        belt[i].is_on = false; //초기에는 로봇이 없으므로 모두 false로 저장
    }
    // 연산 & 출력
