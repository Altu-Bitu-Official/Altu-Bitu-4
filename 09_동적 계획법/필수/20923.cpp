...
int whoCanRingTheBell(cards& deck, cards& ground) {
    if (!ground[DO].empty() && ground[DO].front() == 5) { // 도도가 칠수 있는 경우
        return DO;
    }
    else if (!ground[SU].empty() && ground[SU].front() == 5) { // 도도가 칠 수 있는 경우
        return DO;
    }
    else if (!ground[DO].empty() && !ground[SU].empty() && (ground[DO].front() + ground[SU].front() == 5)) { // 수연이가 칠 수 있는 경우
        return SU;
    }
    return -1; // 종을 울릴 수 없음
}
...
string game(int m, cards& deck, cards& ground) {
    ...
    while (m--)
    {
        ...
        deck[turn].pop_front();
        if (deck[turn].empty()) { // deck이 비면 break
            break;
        }
