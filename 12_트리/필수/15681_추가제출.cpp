...
void dfs(int cur, int prev, vector<vector<int>>& tree, vector<int>& dp) {
	...
	for (int i = 0; i < tree[cur].size(); i++) { // 현재 정점의 자식 노드 탐색
		...
		dfs(tree[cur][i], cur, tree, dp);//각 노드를 루트로 하는 서브트리 정점 수를 구하기 위해 재귀적 호출
...
int main() {
	...
	// 입력
	int n, r, q, u, v;
	cin >> n >> r >> q; //n: 정점수, r:루트 번호, q:쿼리 수

	vector<vector<int>>tree(n + 1, vector<int>(0));

	for (int i = 0; i < n - 1; i++) {
		cin >> u >> v;
		tree[u].push_back(v);//방향성이 없으므로 양쪽에 다 넣어줌
		tree[v].push_back(u);
	}
