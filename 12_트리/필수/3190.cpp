...
int game(int n, vector<vector<int>> &state, map<int, int> &direction) {
    ...
    while (true) {
        x = snake.front().first;  // 뱀의 머리 x 좌표
        y = snake.front().second;  // 뱀의 머리 y 좌표
        if (direction[time] != 0) { // 뱀의 방향 변환 정보가 있으면
            dir = (dir + direction[time]) % 4; // 뱀의 방향 갱신
        }
        time++; // 게임 시간 1초 추가
        nx = x + dx[dir]; // 뱀 머리의 다음 x 좌표
        ny = y + dy[dir]; // 뱀 머리의 다음 x 좌표
...
int main() {
    int n, k, l, x;
    char c;
    vector<vector<int>> state;  // 해당 위치의 상태 (0,1,2)
    map<int, int> direction;
    
    // 입력
    cin >> n >> k;  // 보드의 크기 n, 사과의 개수 k
    state.assign(n + 1, vector<int>(n + 1, 0));
    while (k--) {
        int a, b;
        cin >> a >> b;  // 사과의 x좌표 a, 사과의 y좌표 b
        state[a][b] = 1; // 사과 위치 표시
    }

    cin >> l;   // 뱀의 방향 변환 횟수
    while (l--) {
        cin >> x >> c;  // 게임 시작 시간으로부터 x초가 끝난 뒤에 
        if (c == 'D') { // c가 D이면
