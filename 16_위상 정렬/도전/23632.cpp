    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < resources[x].size(); i++) {
            ...
            for (int j = 0; j < graph[resource].size(); j++) {
                ...
                indegree[building]--; // 재료가 하나 충족되었으므로 진입 차수 감소
...
/*
 * 건물마다 필요로 하는 자원의 관계를 그래프로 표현하여 위상 정렬 수행
 * 이미 지어진 건물은 자원을 생성하고, 필요한 자원이 모두 충족되면 새로운 건물을 지을 수 있음 : 건물 -> 자원 -> 건물
...
int main()
{
    ...
    vector<vector<int>> resources(n + 1, vector<int>(0)); // 각 건물이 생산하는 자원 목록
