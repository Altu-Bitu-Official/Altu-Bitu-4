...
void dfs(int row, int col, int dir) {
    // 1. 현재 위치 청소
    if (board[row][col] != CLEAN) {// 청소되어 있지 않은 상태라면
        cnt++; //청소한칸 개수+1
    }
    board[row][col] = CLEAN;//해당칸을 청소한 것으로 처리

    // [현재 칸의 주변 4칸 중 청소되지 않은 빈 칸이 있는가]
    // 3. 현재 칸의 주변 4칸 중 청소되지 않은 빈 칸이 있는 경우
    for (int i = 0; i < 4; i++) { // 3-1. 반시계 방향으로 90º 회전
        int new_dir = (dir - i + 3) % 4;
        int new_row = row + dy[new_dir], new_col = col + dx[new_dir]; //움직일 방향 탐색

        if (board[new_row][new_col] == 0) { // 3-2. 아직 청소되지 않은 빈 칸 발견
            dfs(new_row, new_col, new_dir); // 한 칸 전진
            return;//청소기 위치 이동 후 리턴
        }
    }

    // 2. 현재 칸의 주변 4칸 중 청소되지 않은 빈 칸이 없는 경우
    int back_dir = (dir + 2) % 4;
    int back_row = row + dy[back_dir], back_col = col + dx[back_dir];//한칸 후진
...
int main() {
    ...
    for (int i = 0; i < n; i++) {//청소여부와 벽 여부를 입력받음
        for (int j = 0; j < m; j++) {
            cin >> board[i][j];
        }
    }
