...
// 기존에 작성했던 코드가 틀렸던 이유 : 알파벳의 등장 빈도를 고려하지 않았다. 

int countDiff(string word, vector<int> original_freq) {
    vector<int> freq(NUM_CHARS, 0); // 비교할 단어에서 각 알파벳이 등장하는 빈도를 기록한다.
    int diff = 0; // 원본 단어와의 차이
    
    countFreq(word, freq); // 비교할 단어의 각 알파벳의 개수 세기
...
int main() {
    ...
    cin >> N;
    cin >> original;
    vector<int> original_freq(NUM_CHARS, 0); // 기준이 되는 단어에서 각 알파벳이 등장한 빈도를 기록한다.
