...
void findMax(int idx_1, int idx_2, int m, int n, int cnt, int cursum, int sum, vector<vector<int>> &terr){
    if (cnt == 4){ // 종료 조건
        cout << cursum << " ";
        if (cursum > sum){
            sum = cursum; // sum 갱신
    ...
    for (int i=0; i<3; i++){
        if ((0<= idx_1 + dr[i] < m) && (0 <= idx_2 + dc[i]) && (idx_2 + dc[i] < n)  && (!visited[idx_1 + dr[i]][idx_2 + dc[i]])){
