...
const int SIZE = 100; // 평면 가로, 세로 길이의 최댓값

// 방향: 우(0), 상(1), 좌(2), 하(3)
int dy[4] = { 0, -1, 0, 1 }; // 상하 방향
int dx[4] = { 1, 0, -1, 0 }; // 좌우 방향

// 1x1 정사각형 개수 계산
int cntSquares(vector<vector<bool>>& plane) {
	int ans = 0; //  ans 0으로 초기화
	for (int i = 0; i < SIZE; i++) {     // 최대 y 길이만큼
		for (int j = 0; j < SIZE; j++) { // 최대 x 길이만큼
...
void drawDragonCurve(vector<vector<bool>>& plane, int x, int y, int d, int g) {
	...
	plane[y][x] = plane[y + dy[d]][x + dx[d]] = true; // 평면에 표시 (시작 방향대로 초기화)
	x += dx[d]; // 시작 방향대로 x좌표 업데이트
	y += dy[d]; // 시작 방향대로 y좌표 업데이트
	direct.push_back(d); // direct에 d 추가
	while (g--) { // 1 ~ g 세대
		int size_d = direct.size(); // 방향의 size
		for (int j = size_d - 1; j >= 0; j--) { // 방향 계산
			int next_d = (direct[j] + 1) % 4; // 다음 방향
			x += dx[next_d]; // 다음 방향대로 x좌표 업데이트
			y += dy[next_d]; // 다음 방향대로 y좌표 업데이트
			plane[y][x] = true; // 평면에 표시
			direct.push_back(next_d); // direct에 다음 방향 추가
		}
	}
}
...
int main()
{
	...
	// 입력
	cin >> n; // 정사각형의 네 꼭짓점이 모두 드래곤 커브의 일부인 것의 개수

	// 연산
	while (n--) { // n번 동안
		cin >> x >> y >> d >> g; // 드래곤 커브의 시작점, 시작 방향, 세대
		drawDragonCurve(plane, x, y, d, g); // 드래곤 커브 그리기
	}
